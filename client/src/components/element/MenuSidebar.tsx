...
export const MenuSidebar: React.FC<PageProps> = ({ icons, title, url }) => {
    ...
    return (
        <Link to={url} className={`${location.pathname === url ? 'bg-blue-500 text-white' : 'bg-transparent'}
            h-10 w-full rounded flex items-center cursor-pointer hover:bg-blue-500 hover:text-white transition-all`}>
